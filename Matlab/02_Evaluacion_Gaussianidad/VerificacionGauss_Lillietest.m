...
for k=1:M   % Number Class
    ...
    for i=1:d  % Number caracteristics
        x=X(:,i);
        [h,p] = lillietest(x,'Alpha',alpha); %Lillietest: h=0 gaussian,  h=1 NO gaussian
        H(k,d)=h;
    end    
end

% Display test outcomes
f = figure('Position',[440 500 461 146],'Name',['Lillietest  alpha=' num2str(alpha)] );

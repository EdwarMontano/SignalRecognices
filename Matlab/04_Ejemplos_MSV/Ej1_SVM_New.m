...
% Seleccionar Optimización Solver
Solut= questdlg('Select an Optimization Routine Solver','Optimization','ISDA', 'L1QP','SMO','ISDA');

% Remove Duplicates of data set
remdup = questdlg('Remove Duplicates of Data Set?', 'Option', 'on','off','off');

% Train an SVM classifier using the processed data set.
switch kernel
    case 'linear'
        %SVMModel = fitcsvm(Xtrain,ytrain,'BoxConstraint',500,'RemoveDuplicates',remdup,'Cost',Costo, 'Prior', APriori ,'KernelFunction', kernel, 'Solver', Solut);        
...
colorbar;
title('{\bf Data Set Training Dispertion- SupportVector}')
...
%% Plot a scatter diagram of the data and the support vectors.
...
figure
plotconfusion(Target,Estimat); title('Confusion Matrix - Test Data Set')

...
N =50;
porcentaje_train = 50;
N_datos = round(N*porcentaje_train/100); %los datos de entrenamiento serán los primeros N_datos
c1=3;c2=4; % se eligen 2 de las 4 características de ficheriris que se usarán para el ejemplo
...
% Se calcula la media y la varianza de los datos de entrenamient para encontrar outliers
 ...
 % se define el múltiplo de las desviaciones estandar a partir del cuales los datos
...
scatter(X1,X2,N_datos,C,'marker','*'); %dibuja asteríscos por cada dato, con un color dependiendo de la clase
hold on;grid on;
 ...
 for c=1:3
     ...
     h = rectangle('position',[m(:,c)'-d(:,c)',2*d(1,c),2*d(2,c)], 'curvature',[0 0]);
        set(h,'linestyle',':') % dibuja un rectángulo hasta donde se cumle la condición de las desviaciones estándar
 end
 title(strcat('Datos con  ',num2str(veces),' veces la desviación estándar'));
 ...
 for n=1:fin
     ...
     if outlier == 0 %caso no es outlier, se marca con un círculo verde para identificarlo gráficamente
         X_train(:,k) = X_datos(:,n); %se adiciona al vector de datos a ser usados
         C2(k) = C(n); 
         k=k+1;
         plot(X_datos(1,n),X_datos(2,n),'go');
     else % caso es un outlier, se marca con un círculo rojo para identificarlo gráficamente, y no se adiciona a los datos a ser usados

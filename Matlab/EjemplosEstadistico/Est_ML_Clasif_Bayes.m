...
% Generate Xp (prueba)
randn('seed',100);
[Xp,yp]=generate_gauss_classes(m,S,P,N);
...
% Display Xp (Prueba)
subplot(2,1,2); plot3(Xp(1,1:N1),Xp(2,1:N1),Xp(3,1:N1),'or',Xp(1,N1+1:N1+N2),...
    Xp(2,N1+1:N1+N2),Xp(3,N1+1:N1+N2),'ob', Xp(1,N1+N2+1:N1+N2+N3),...
    Xp(2,N1+N2+1:N1+N2+N3),Xp(3,N1+N2+1:N1+N2+N3),'ok'); 
...
z_euclidean=euclidean_classifier(m_est,Xp);

%% 3. the Mahalanobis distance classifier using the ML estimates
z_mahalanobis=mahalanobis_classifier(m_est,S_est,Xp);

%% 4. Bayesian classifier, using the ML estimates, in order to
% classify the data vectors of Xp
z_bayesian=bayes_classifier(m_est,S_est,P,Xp);
...
z_bayesian_Orig=bayes_classifier(m,S,P,Xp);

...
Pw1T= NT1/(NT1+NT2); Pw2T= NT2/(NT1+NT2); 
...
for i=1:length(T)
    x= T(i);
    %obtener P(w1|x) --> A Posteriori Probability for clase 1 
    if x<minr1 || x>maxr1 
        APwx1=0;
    else
        ind1=ceil((x-minr1)/delta1);
        %APwx1=pw1x(ind1)*Pw1T;
        APwx1=pw1x(ind1);
    end    
    
    %obtener P(w2|x)--> A Posteriori Probability for clase 2
    if x<minr2 || x>maxr2 
        APwx2=0;
    else
        ind2=ceil((T(i)-minr2)/delta2);
        %APwx2=pw2x(ind2)*Pw2T;
        APwx2=pw2x(ind2);
    end    
    
    % Clasificar Usando Decisió Bayesiana
